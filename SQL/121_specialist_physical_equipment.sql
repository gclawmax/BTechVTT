...
BEGIN
 ...
 patched:=replace(source,
  ...
  'target_damage:=ceil(attacker_mass*3/10.0)::int;IF btech_talon_operational(g.catalogue_version,attacker,''ll'') OR btech_talon_operational(g.catalogue_version,attacker,''rl'') THEN target_damage:=ceil(target_damage*1.5)::int;END IF; /* sr6b_talon_dfa_v1 */ remaining:=target_damage;');
