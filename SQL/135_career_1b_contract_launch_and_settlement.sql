...
BEGIN
 ...
 -- Avoid requiring the optional pgcrypto extension: md5/random/clock_timestamp
 -- are PostgreSQL built-ins and are sufficient for a human-facing match code.
 LOOP new_game_code:='BT-C'||upper(substr(md5(random()::text||clock_timestamp()::text),1,4));EXIT WHEN NOT EXISTS(SELECT 1 FROM btech_games g WHERE g.game_code=new_game_code);END LOOP;
