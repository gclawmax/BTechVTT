...
try {
  ...
  check('the deployed browser includes the AI specialist authority',/^20260906-ai[5-7]-/.test(result.build)||/^20260907-(game-modes|gm2-ai-skirmish)-/.test(result.build),result.build);
  if(/^20260906-ai[67]-/.test(result.build)||/^20260907-(game-modes|gm2-ai-skirmish)-/.test(result.build)) check('AI difficulty and personality persist into the match',result.createdDifficulty==='advanced'&&result.createdPersonality==='sniper'&&result.plannedPersonality==='sniper',JSON.stringify({difficulty:result.createdDifficulty,personality:result.createdPersonality,planned:result.plannedPersonality}));
