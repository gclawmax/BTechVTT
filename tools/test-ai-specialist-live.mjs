...
try {
  ...
  const result = await page.evaluate(async () => {
    ...
    return {
      build:BT_BUILD_ID, specialistHandlers:typeof executeAIReaction==='function' && typeof executeAIPhysicalAttack==='function', gameId:currentGameId, gameCode:game.game_code, catalogueVersion:game.catalogue_version,
  ...
  check('the deployed browser includes the AI specialist authority',result.specialistHandlers,result.build);
  check('AI difficulty and personality persist into the match',result.createdDifficulty==='advanced'&&result.createdPersonality==='sniper'&&result.plannedPersonality==='sniper',JSON.stringify({difficulty:result.createdDifficulty,personality:result.createdPersonality,planned:result.plannedPersonality}));
