...
const runs = Math.max(1,Math.min(100,Number(process.env.BT_AI_SOAK_RUNS || 10)) || 10);
...
const reportDir = join(tmpdir(),'btechvtt-ai-weapon-soak');
...
if (process.env.BT_AI_SOAK_LIST === '1') {
  console.log(`AI-2 weapon soak: ${runs} Play-vs-AI activation(s), seed ${seedBase}.`);
...
try {
  ...
  for (let index = 1; index <= runs; index++) {
    ...
    try {
      ...
      },`AI-2 Play-vs-AI soak ${index}/${runs}`);
    } catch (error) {
  ...
  await writeFile(summaryPath,JSON.stringify({ generatedAt:new Date().toISOString(),runs,seedBase,passed:failures.length === 0,failures,reports },null,2));
  if (failures.length) {
