...
(async()=>{
 ...
 try {
  ...
  await page.evaluate(()=>{resolveMechLabCatalogueVersion=async()=> 'ui-test';loadCustomSavedDesigns=async()=>{};showScreen('menu-screen');});
  await page.evaluate(()=>openMechDesigner());
  await page.locator('#mech-designer-screen.active h2').waitFor();
  assert.equal(await page.locator('.designer-profile-note').count(),1);
  ...
  await page.evaluate(async()=>{currentGameId='ui-test';activeCatalogueVersion='ui-test';loadLobbyUI=()=>{};await openMechDesigner();});
  await page.getByRole('button',{name:'Back',exact:true}).click();
