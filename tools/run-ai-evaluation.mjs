...
try{
  ...
  console.log(`Balance: ${Object.entries(result.summary.byVictory).map(([mode,item])=>{const decisive=Number(item.pairedSeatOneAdvantages||0)+Number(item.pairedSeatTwoAdvantages||0);return `${mode} paired seat 1 ${item.pairedSeatOneAdvantageRate===null?'n/a':`${item.pairedSeatOneAdvantageRate}%`}${item.pairedComparisons?` (${decisive} decisive / ${item.pairedComparisons} pairs)`:''} · score gap ${item.averageScoreDifferential} · round-limit ${item.roundLimitAdjudicationRate}%${item.averageTimeToObjective===null?' · no objective score':` · first score R${item.averageTimeToObjective}`}`;}).join(' | ')}`);
