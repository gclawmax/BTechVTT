...
function contentPack(registry) {
  const lines = cataloguePreamble(registry);
  for (const unit of registry.units) lines.push(...unitStatements(registry, unit));
  lines.push('COMMIT;','');
  return lines.join('\n');
}

function cataloguePreamble(registry, partLabel = null) {
  return [
    ...
    ...(partLabel ? [`-- Supabase SQL Editor catalogue part ${partLabel}. Safe to rerun.`] : []),
    'BEGIN;',
...
function unitStatements(registry, unit) {
  const lines = [`INSERT INTO public.btech_catalogue_units(catalogue_version,unit_id,source_uuid,definition) VALUES (${sql(registry.catalogue_version)},${sql(unit.id)},${unit.source_uuid ? sql(unit.source_uuid) : 'NULL'},${jsonSql(unit.definition)}) ON CONFLICT (catalogue_version,unit_id) DO NOTHING;`];
...
function contentPackParts(registry, maxBytes = 400_000) {
  const unitBlocks = registry.units.map(unit => unitStatements(registry, unit));
  const chunks = [];
  let current = [];
  for (const block of unitBlocks) {
    const candidate = [...cataloguePreamble(registry, 'pending'), ...current, ...block, 'COMMIT;', ''].join('\n');
    if (current.length && Buffer.byteLength(candidate) > maxBytes) {
      chunks.push(current);
      current = [];
    }
    current.push(...block);
  ...
  const files = chunks.map((statements, index) => {
    const number = String(index + 1).padStart(width, '0');
    const total = String(chunks.length).padStart(width, '0');
    return { name: `${number}_of_${total}.sql`, sql: [...cataloguePreamble(registry, `${number} of ${total}`), ...statements, 'COMMIT;', ''].join('\n') };
  });
  const counts = {
    units: registry.units.length,
    mounts: registry.units.reduce((total, unit) => total + unit.mounts.length, 0),
    slots: registry.units.reduce((total, unit) => total + Object.values(unit.criticals).flat().filter(Boolean).length, 0),
    ammo: registry.units.reduce((total, unit) => total + unit.ammo_bins.length, 0)
  };
  const verifyNumber = String(chunks.length + 1).padStart(width, '0');
  const version = sql(registry.catalogue_version);
  const verification = `-- Run after every numbered catalogue part. Safe to rerun.\nDO $$\nDECLARE unit_count int;mount_count int;slot_count int;ammo_count int;\nBEGIN\n SELECT count(*) INTO unit_count FROM public.btech_catalogue_units WHERE catalogue_version=${version};\n SELECT count(*) INTO mount_count FROM public.btech_catalogue_mounts WHERE catalogue_version=${version};\n SELECT count(*) INTO slot_count FROM public.btech_catalogue_critical_slots WHERE catalogue_version=${version};\n SELECT count(*) INTO ammo_count FROM public.btech_catalogue_ammo_bins WHERE catalogue_version=${version};\n IF unit_count<>${counts.units} OR mount_count<>${counts.mounts} OR slot_count<>${counts.slots} OR ammo_count<>${counts.ammo} THEN\n  RAISE EXCEPTION 'Catalogue import incomplete: units %/${counts.units}, mounts %/${counts.mounts}, slots %/${counts.slots}, ammo %/${counts.ammo}',unit_count,mount_count,slot_count,ammo_count;\n END IF;\nEND $$;\n`;
  files.push({ name: `${verifyNumber}_verify.sql`, sql: verification });
  return files;
}
...
async function main() {
  ...
  const parts = contentPackParts(registry, partBytes);
  await mkdir(partsPath, { recursive:true });
  for (const entry of await readdir(partsPath, { withFileTypes:true })) {
    if (entry.isFile() && entry.name.endsWith('.sql')) await unlink(join(partsPath, entry.name));
  }
  for (const part of parts) await writeFile(join(partsPath, part.name), part.sql);
  console.log(`Wrote ${parts.length - 1} SQL Editor parts plus ${parts.at(-1).name} to ${partsPath}.`);
}
