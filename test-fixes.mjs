...
check('#5 replay viewer imports only sealed exports and replays their local event timeline', indexSource.includes('openReplayViewer()') && /js\/game\/replay-viewer\.js\?v=/.test(indexSource) && replayViewerClient.includes("parsed?.format !== 'btvtt-replay-1'") && replayViewerClient.includes('replayStateAt') && replayViewerClient.includes('toggleReplayPlayback'));
