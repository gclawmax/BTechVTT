...
check('#5 clients load the shared sealed telemetry envelope on completion', indexSource.includes('js/game/telemetry.js?v=20260902-replay-viewer-49') && telemetryClient.includes("db.rpc('get_btech_match_report'") && fs.readFileSync(`${ROOT}/js/game/phases.js`, 'utf8').includes('await loadSealedMatchReport()') && heatSource.includes('sealedTelemetryStatusText'));
...
check('#5 selected BattleMech sidebar can be collapsed and resized locally', indexSource.includes('id="btn-toggle-mech-panel"') && panelResizeSource.includes('function toggleMechPanel') && panelResizeSource.includes('--detail-panel-width') && panelResizeSource.includes('bt-vtt-detail-panel-collapsed'));
...
check('#5 scenario catalogue repair has a visible, cache-busting build marker', indexSource.includes('20260826-dropship-navigation-22'));
