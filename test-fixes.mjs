...
check('#5 dropship and fixed build stamps share the one visible release marker', indexSource.includes('data-build="20260901-rejoin-slots-47"') && indexSource.includes('id="map-build-stamp"') && supabaseSource.includes("document.body.dataset.build") && supabaseSource.includes("#bt-build-stamp, #map-build-stamp"));
...
check('#5 clients load the shared sealed telemetry envelope on completion', indexSource.includes('js/game/telemetry.js?v=20260901-rejoin-slots-47') && telemetryClient.includes("db.rpc('get_btech_match_report'") && fs.readFileSync(`${ROOT}/js/game/phases.js`, 'utf8').includes('await loadSealedMatchReport()') && heatSource.includes('sealedTelemetryStatusText'));
check('#5 rejoin-critical slot labels are defined by the early core helper', fs.readFileSync(`${ROOT}/js/core/helpers.js`, 'utf8').includes('function criticalSlotName(slot)') && !fs.readFileSync(`${ROOT}/js/game/critical-hits.js`, 'utf8').includes('function criticalSlotName(slot)'));
