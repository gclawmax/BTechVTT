...
const c3Attacker = { ...aDefault, instanceId: 'c3-attacker', unitId: 'c3slave', col: 0, row: 0, c3Network:{id:'p1-A',type:'standard',role:'slave',parentInstanceId:'c3-source'} };
const c3Source = { ...aDefault, instanceId: 'c3-source', unitId: 'c3master', col: 10, row: 0, c3Network:{id:'p1-A',type:'standard',role:'master',parentInstanceId:null} };
...
check('#3 prone firing permits an intact supporting arm when the other arm is destroyed', sandbox.evaluateWeaponAttack({ ...proneShooter, structure: { ...STRUCT, ra: 0 } }, t3, { ...wpn, location: 'Center Torso' }).valid);
...
const electronicDesign = { ...baseCustomDesign, weapons:[{key:'med_laser',location:'ra'},{key:'ppc',location:'rt'}], electronics:[{key:'is_targeting_computer',location:'lt'},{key:'guardian_ecm',location:'lt'},{key:'c3_slave',location:'head'}] };
const electronicCalculation = sandbox.calculateCustomDesign(electronicDesign);
check('#4d electronic equipment uses mass, slots and variable Targeting Computer sizing', electronicCalculation.valid && electronicCalculation.weights.electronics === 4.5 && electronicCalculation.slots.lt === 6 && electronicCalculation.slots.head === 0, JSON.stringify(electronicCalculation));
...
check('#5 deployment uses the battlefield’s staggered hex geometry rather than square cells', lobbySource.includes('function deploymentHexPoints') && lobbySource.includes('deploymentMapMarkup(gameState') && fs.readFileSync(`${ROOT}/js/ui/deployment-map.js`, 'utf8').includes('drawMapHex(point.x*HEX_SIZE') && mainCssSource.includes('.deployment-map') && mainCssSource.includes('.deployment-hex { fill:'));
check('#5 Create Match previews use staggered terrain hexes rather than square cells', createGameSource.includes('function mapPreviewHexPoints') && createGameSource.includes('<svg class="map-preview-grid"') && createGameSource.includes('<polygon class="map-preview-hex') && mainCssSource.includes('.map-preview-hex { fill:'));
...
const telemetryClient = fs.readFileSync(`${ROOT}/js/game/telemetry.js`, 'utf8');
check('#5 telemetry records immutable ordered server events and replay checkpoints', telemetryMigration.includes('CREATE TABLE IF NOT EXISTS public.btech_match_telemetry') && telemetryMigration.includes('UNIQUE(game_id,event_index)') && telemetryMigration.includes("'state_checkpoint'") && telemetryMigration.includes('btech_replay_state_snapshot'));
check('#5 telemetry extracts purpose-labelled dice and damage attribution from authoritative combat results', telemetryMigration.includes('btech_collect_dice_rolls') && telemetryMigration.includes("'purpose',array_to_string(p_path,'.')") && telemetryMigration.includes('btech_combat_damage_attribution') && telemetryMigration.includes("NEW.phase||'_resolved'"));
check('#5 movement, facing, heat, pilot and damage changes are captured from authoritative state updates', ['movement_or_facing','heat','pilot','damage'].every(kind => telemetryMigration.includes(`'\"${kind}\"'::jsonb`)) && telemetryMigration.includes("'unit_state_changed'"));
check('#5 completed matches receive a versioned sealed report without exposing the game code', telemetryMigration.includes("'btvtt-match-report-1'") && telemetryMigration.includes('btech_seal_match_report') && telemetryMigration.includes('get_btech_match_report') && !telemetryMigration.includes("'game_code',"));
check('#5 clients load the shared sealed telemetry envelope on completion', /js\/game\/telemetry\.js\?v=/.test(indexSource) && telemetryClient.includes("db.rpc('get_btech_match_report'") && fs.readFileSync(`${ROOT}/js/game/phases.js`, 'utf8').includes('await loadSealedMatchReport()') && heatSource.includes('sealedTelemetryStatusText'));
const replayViewerClient = fs.readFileSync(`${ROOT}/js/game/replay-viewer.js`, 'utf8');
check('#5 replay viewer imports only sealed exports and replays their local event timeline', indexSource.includes('openReplayViewer()') && indexSource.includes('js/game/replay-viewer.js?v=20260902-replay-viewer-49') && replayViewerClient.includes("parsed?.format !== 'btvtt-replay-1'") && replayViewerClient.includes('replayStateAt') && replayViewerClient.includes('toggleReplayPlayback'));
const helperSource = fs.readFileSync(`${ROOT}/js/core/helpers.js`, 'utf8');
check('#5 rejoin-critical slot labels are defined by the early core helper', helperSource.includes('function criticalSlotName(slot)') && !fs.readFileSync(`${ROOT}/js/game/critical-hits.js`, 'utf8').includes('function criticalSlotName(slot)'));
check('#5 retained finished skirmishes show a player result and reopen their report', helperSource.includes('Recent Finished Games') && helperSource.includes('Finished — Win') && helperSource.includes('Finished — Loss') && helperSource.includes('Finished — Draw') && helperSource.includes("game.status === 'in-progress' || game.status === 'finished'"));
const statisticsMigration = fs.readFileSync(`${ROOT}/SQL/101_authoritative_match_statistics.sql`, 'utf8');
const resolverRepairMigration = fs.readFileSync(`${ROOT}/SQL/102_repair_dfa_and_special_ammunition_function.sql`, 'utf8');
check('#5 match statistics are calculated server-side from canonical telemetry resolutions', statisticsMigration.includes('btech_build_match_statistics') && statisticsMigration.includes('DISTINCT ON (payload->>\'combat_event_id\')') && statisticsMigration.includes('btech_two_d6_success_probability') && statisticsMigration.includes("'btvtt-match-report-2'"));
check('#5 battle report shows sealed statistics, expectation-aware dice, and the complete saved log', indexSource.includes('victory-report-overlay') && telemetryClient.includes('reportExpectation') && telemetryClient.includes('loadBattleReportLog') && telemetryClient.includes('openVictoryReport'));
check('#5 DFA catalogue lookup and special ammunition helper are repaired without ambiguous columns', resolverRepairMigration.includes('unit_definition jsonb') && resolverRepairMigration.includes('SELECT cu.definition INTO unit_definition') && resolverRepairMigration.includes('btech_apply_special_ammo_damage(jsonb,int,text,boolean,text,text)'));
const afterActionMigration = fs.readFileSync(`${ROOT}/SQL/103_after_action_preview_export_and_retention.sql`, 'utf8');
check('#5 sealed skirmish reports provide read-only Career previews and portable export contracts', afterActionMigration.includes('get_btech_skirmish_career_preview') && afterActionMigration.includes("g.match_type<>'skirmish'") && afterActionMigration.includes('get_btech_match_report_export') && afterActionMigration.includes('get_btech_match_replay_export') && afterActionMigration.includes('btvtt-replay-1'));
check('#5 retention removes only completed aged skirmishes and never grants browser cleanup access', afterActionMigration.includes("g.match_type='skirmish'") && afterActionMigration.includes("g.completed_at<p_before") && afterActionMigration.includes("status<>'in-progress'") && afterActionMigration.includes('REVOKE ALL ON FUNCTION public.run_btech_skirmish_retention_cleanup'));
check('#5 battle report exposes export buttons and makes the Career preview visibly non-persistent', telemetryClient.includes('exportBattleReplay') && telemetryClient.includes('get_btech_skirmish_career_preview') && telemetryClient.includes('Skirmish only — not saved'));
const heatLedgerRepairMigration = fs.readFileSync(`${ROOT}/SQL/104_correct_authoritative_heat_ledger.sql`, 'utf8');
check('#5 authoritative weapon heat is added once and Heat Management recomputes the round ledger', heatLedgerRepairMigration.includes('heat_ledger_no_double_weapon_v1') && heatLedgerRepairMigration.includes('heat_ledger_recomputed_v1') && heatLedgerRepairMigration.includes('heat_ledger_override_v1') && heatSource.includes('const expectedHeat = starting + movement + weapons'));
const specialAttackRepairMigration = fs.readFileSync(`${ROOT}/SQL/105_repair_charge_catalogue_lookup.sql`, 'utf8');
check('#5 Charge declaration and special-attack catalogue mass lookups cannot be ambiguous', specialAttackRepairMigration.includes('charge_catalogue_alias_v1') && specialAttackRepairMigration.includes('SELECT cu.definition INTO unit_definition') && specialAttackRepairMigration.includes('resolve_declared_charge_legacy') && specialAttackRepairMigration.includes('resolve_declared_death_from_above_legacy'));
const physicalSkipMigration = fs.readFileSync(`${ROOT}/SQL/106_skip_unavailable_physical_attacks.sql`, 'utf8');
check('#5 Physical Attack skips only when no legal special, kick, punch, or physical-equipment option remains', physicalSkipMigration.includes('btech_has_remaining_physical_option') && physicalSkipMigration.includes("attack_diff IN (0,1,5)") && physicalSkipMigration.includes("attack_diff<>3") && physicalSkipMigration.includes('weapon_mounts') && physicalSkipMigration.includes("current_phase='heat'"));
const specialAttackCompletionMigration = fs.readFileSync(`${ROOT}/SQL/107_complete_charge_and_dfa_rules.sql`, 'utf8');
check('#5 Charge and DFA validate their plotted movement path and preserve the pinned catalogue', specialAttackCompletionMigration.includes('btech_validate_special_attack_path') && specialAttackCompletionMigration.includes('p_catalogue_version') && specialAttackCompletionMigration.includes('Charge path cannot enter an occupied hex') && movementSource.includes("p_path: moveState.path || []"));
check('#5 Charge and DFA miss after a weapon-phase fall or unconscious pilot, and DFA misses fall two levels to the rear', specialAttackCompletionMigration.includes('charge_complete_rules_v1') && specialAttackCompletionMigration.includes('dfa_complete_rules_v1') && specialAttackCompletionMigration.includes('fall_damage:=ceil(attacker_mass/10.0)::int*2') && specialAttackCompletionMigration.includes("btech_roll_mech_hit_location(''rear'')"));
...
const panelResizeSource = fs.readFileSync(`${ROOT}/js/ui/panel-resize.js`, 'utf8');
check('#5 selected BattleMech summary keeps tactical coordinates in a collapsed footer', panelSource.includes('<details class="mech-technical-details">') && panelSource.indexOf('class="armor-diagram"') < panelSource.indexOf('class="mech-technical-details"') && panelSource.includes('Position &amp; facing'));
check('#5 armour and structure tiles use full-tile condition colours and enlarged current values', panelSource.includes('background:${background};border-color:${border}') && panelSource.includes('A <strong>${armor') && mainCssSource.includes('.armor-cell .val strong { font-size:14px; }'));
check('#5 selected BattleMech sidebar can be collapsed and resized locally', indexSource.includes('id="btn-toggle-mech-panel"') && panelResizeSource.includes('function toggleMechPanel') && panelResizeSource.includes("classList.toggle('mech-panel-collapsed'") && panelResizeSource.includes('bt-vtt-detail-panel-collapsed') && mainCssSource.includes('.mech-panel-collapsed') && mainCssSource.includes('--detail-panel-width'));
check('#5 compact record statistics keep labels and values on one grid row', mainCssSource.includes('.mech-summary-grid { grid-template-columns:max-content minmax(0, 1fr)') && mainCssSource.includes('.mech-summary-grid .k { white-space:nowrap; }'));
...
check('#5 scenario catalogue repair has a visible, cache-busting build marker', /<body[^>]*data-build="[^"]+"/i.test(indexSource) && /<script[^>]*\?v=[^"\s]+/.test(indexSource) && indexSource.includes('id="bt-build-stamp"') && indexSource.includes('id="map-build-stamp"'));
...
const individualAmmoMigration = fs.readFileSync(`${ROOT}/SQL/155_individual_ammunition_setup_recovery.sql`, 'utf8');
check('#5 each Round 1 ammunition button commits only its own physical bin',
  individualAmmoMigration.includes('CREATE OR REPLACE FUNCTION public.confirm_round_one_ammunition_bin(p_game_id uuid,p_bin_key text,p_load_type text)')
  && individualAmmoMigration.includes('IF provided<>1 THEN RAISE EXCEPTION')
  && individualAmmoMigration.includes("'This ammunition bin is already confirmed'")
  && individualAmmoMigration.includes("'An ammunition selection does not belong to one of your configurable bins'")
  && phasesSource.includes('async function submitRoundOneAmmoLoadout(binKey)')
  && phasesSource.includes('const entries = pendingEntries.filter(([key]) => key === binKey)')
  && phasesSource.includes('if (entries.length !== 1) return;')
  && phasesSource.includes("db.rpc('confirm_round_one_ammunition_bin'")
  && panelSource.includes('Confirm this ammunition bin'));
...
const roundHeatMigration = fs.readFileSync(`${ROOT}/SQL/98_round_heat_ledger_reset.sql`, 'utf8');
check('#5 a new round promotes post-sink heat and clears every per-round heat source', roundHeatMigration.includes("'{roundStartingHeat}', to_jsonb(COALESCE((value->>'heat')::int, 0))") && roundHeatMigration.includes("'{movementHeat}', '0'::jsonb") && roundHeatMigration.includes("'{weaponHeat}', '0'::jsonb") && roundHeatMigration.includes("'{externalHeat}', '0'::jsonb") && roundHeatMigration.includes("'{pendingTerrainHeat}', '0'::jsonb"));
...
check('#5 electronic construction is repeated authoritatively by SQL', declaredNetworkMigration.includes('btech_custom_targeting_computer_tons') && declaredNetworkMigration.includes("WHEN 'guardian_ecm'") && declaredNetworkMigration.includes("WHEN 'clan_ecm'") && declaredNetworkMigration.includes("WHEN 'c3_master'") && declaredNetworkMigration.includes("WHEN 'c3i'") && declaredNetworkMigration.includes("'{weights,electronics}'"));
check('#5 the lobby persists declared C3 and C3i topology', lobbySource.includes('renderLobbyC3Networks') && lobbySource.includes("db.rpc('set_match_c3_assignments'") && lobbySource.includes('parentInstanceId') && declaredNetworkMigration.includes('A C3 Master may control at most three units') && declaredNetworkMigration.includes('Standard C3 and C3i cannot share a network'));
check('#5 declared networks replace automatic force-wide C3', weaponAttackSource.includes('attacker.c3Network') && weaponAttackSource.includes('pathToRoot') && declaredNetworkMigration.includes('btech_c3_connected_root') && declaredNetworkMigration.includes("THEN 6 ELSE 12") && declaredNetworkMigration.includes("THEN 'master'") && declaredNetworkMigration.includes("THEN 'slave'"));
check('#5 C3 network limits avoid an ambiguous PL/pgSQL CASE terminator', declaredNetworkMigration.includes('INTO network_limit') && declaredNetworkMigration.includes('IF network_count>network_limit') && !declaredNetworkMigration.includes(') > CASE WHEN EXISTS'));
...
const ai5Migration = fs.readFileSync(`${ROOT}/SQL/126_ai_authoritative_reactions_and_physical.sql`, 'utf8');
check('#5 AI reactions and physical attacks use the maintained authoritative resolvers', aiOpponentSource.includes("db.rpc('submit_torso_twist_reaction'") && aiOpponentSource.includes("db.rpc('submit_simultaneous_physical_declaration'") && aiOpponentSource.includes("db.rpc('resolve_push_attack'") && ai5Migration.includes('btech_authorized_ai_phase_player'));
check('#5 AI plans punches, kicks, pushes, specialist physical weapons, Charge and DFA', aiOpponentSource.includes('physicalAttackTypesFor(mech)') && aiOpponentSource.includes("type:'resolve_dfa'") && aiOpponentSource.includes("type:'resolve_charge'") && aiOpponentSource.includes("type:'declare_dfa'") && aiOpponentSource.includes("type:'declare_charge'"));
check('#5 AI supports arm flipping, prone support and improvised clubs', aiOpponentSource.includes('__arms_flipped') && aiOpponentSource.includes('proneSupportArm') && aiOpponentSource.includes('generateAIClubSearchAction') && aiOpponentSource.includes("db.rpc('find_improvised_club'"));
check('#5 AI searches for hidden contacts and avoids only revealed enemy minefields', aiOpponentSource.includes('aiSearchWaypoint') && aiOpponentSource.includes('probeSearchScore') && aiOpponentSource.includes('aiKnownEnemyMinefield') && aiOpponentSource.includes('revealed_to'));
...
check('#5 local settings provide combat-log speed, mute and volume controls', indexSource.includes('id="btn-game-settings"') && indexSource.includes('id="combat-log-speed"') && indexSource.includes('id="sound-effects-mute"') && indexSource.includes('id="sound-effects-volume"') && gameSettingsSource.includes('BT_GAME_SETTINGS_KEY') && gameSettingsSource.includes('localStorage.setItem'));
check('#5 weapon narration groups saved results by BattleMech without changing server resolution', weaponAttackSource.includes("kind: 'weapon-header'") && weaponAttackSource.includes('registerResolvedWeaponEvent(event.id)') && gameSettingsSource.includes('queueAuthoritativeWeaponPresentation') && gameSettingsSource.includes('combatPresentationQueue'));
check('#5 ballistic, missile and laser placeholders are classified and shipped', gameSettingsSource.includes("ballistic: 'assets/audio/ballistic-fire.wav'") && gameSettingsSource.includes("missile: 'assets/audio/missile-fire.wav'") && gameSettingsSource.includes("laser: 'assets/audio/laser-fire.wav'") && ['ballistic-fire.wav','missile-fire.wav','laser-fire.wav'].every(file => fs.statSync(`${ROOT}/assets/audio/${file}`).size > 1000));
check('#5 Start Career saves a persistent account avatar', indexSource.includes('onsubmit="saveCareerAvatar(event)"') && indexSource.includes('id="career-avatar-save"') && careerAvatarSource.includes('function openCareerAvatarCreator') && careerAvatarSource.includes('db.auth.updateUser') && careerAvatarSource.includes('career_avatar: careerAvatar') && careerAvatarSource.includes('create_btech_career_company') && careerAvatarSource.includes('Save Persistent Avatar'));
...
const mapsSource = fs.readFileSync(`${ROOT}/js/game/maps.js`, 'utf8');
const customScenarioMigration = fs.readFileSync(`${ROOT}/SQL/89_custom_map_and_scenario_editor.sql`, 'utf8');
check('#5 map editor paints supported terrain and elevation on the native hex grid', scenarioEditorSource.includes('SCENARIO_TERRAIN') && scenarioEditorSource.includes("type === 'elevation'") && scenarioEditorSource.includes('scenarioEditorHexPoints') && scenarioEditorSource.includes('scenarioEditorDimensions'));
check('#5 variable maps support standard sheets, dual-board presets and seeded fair terrain', scenarioEditorSource.includes("['single', 16, 17") && scenarioEditorSource.includes("['dual-vertical', 16, 34") && scenarioEditorSource.includes("['dual-horizontal', 32, 17") && scenarioEditorSource.includes('seededRandom') && scenarioEditorSource.includes("pattern === 'ridge'") && scenarioEditorSource.includes("pattern === 'river'") && scenarioEditorSource.includes("pattern === 'urban'") && scenarioEditorSource.includes('scenario-symmetry') && mapsSource.includes('function mapDimensions') && mapsSource.includes("'standard-dual-horizontal'"));
...
check('#5 minefield detection uses a complete PL/pgSQL conditional rather than an inline CASE terminator', hiddenMineMigration.includes('detection_target:=CASE') && hiddenMineMigration.includes('IF roll>=detection_target THEN') && !hiddenMineMigration.includes('IF roll>=CASE'));
...
check('#5 advanced AI weighs personality-adjusted MASC risk and uses the authoritative failure resolver', mascAiSource.includes('riskAcceptable') && mascAiSource.includes('doctrine.riskTolerance') && mascAiSource.includes('Math.round(5 +') && mascAiSource.includes("{ ...step, masc: true }") && mascAiSource.includes("db.rpc('submit_battlemech_movement'"));
...
check('#5 duel soak rotates maps and chooses reproducible random supported BattleMechs through the real lobby', ['woodland-approach','open-engagement','flatlands-open-terrain','ridge-and-ford','databaseSupportedUnitIds','BT_SOAK_PROFILE','BT_SOAK_SEED','seeded random'].every(value => humanBattleSource.includes(value)) && duelSoakSource.includes('BT_SOAK_PROFILE') && duelSoakSource.includes('BT_SOAK_SEED'));
