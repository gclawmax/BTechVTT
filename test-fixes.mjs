...
check('#5 dropship and fixed build stamps share the one visible release marker', indexSource.includes('data-build="20260825-shutdown-rules-13"') && indexSource.includes('id="map-build-stamp"') && supabaseSource.includes("document.body.dataset.build") && supabaseSource.includes("#bt-build-stamp, #map-build-stamp"));
...
check('#5 scenario catalogue repair has a visible, cache-busting build marker', indexSource.includes('20260825-shutdown-rules-13'));
...
check('#5 shutdown recovery follows Heat Level rules and makes the conscious-pilot override explicit', heatSource.includes('declareShutdownOverride') && heatSource.includes('restarted automatically after cooling below Heat Level 14') && heatSource.includes('Attempt Shutdown Override'));
