...
check('#5 dropship and fixed build stamps share the one visible release marker', indexSource.includes('data-build="20260825-weapon-allocation-07"') && indexSource.includes('id="map-build-stamp"') && supabaseSource.includes("document.body.dataset.build") && supabaseSource.includes("#bt-build-stamp, #map-build-stamp"));
...
check('#5 scenario catalogue repair has a visible, cache-busting build marker', indexSource.includes('20260825-weapon-allocation-07'));
check('#5 initiative waits for every player to commit Round 1 ammunition', phasesSource.includes('const unconfiguredAmmo') && phasesSource.includes('const ownAmmoSetupPending') && phasesSource.includes('Waiting for the other player to declare their specialised ammunition.'));
