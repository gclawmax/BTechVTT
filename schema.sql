...
-- A player may update only their own roster while a match is still in the
-- lobby. The SECURITY DEFINER function leaves the host-only game UPDATE
-- policy in place for all other game state.
...
-- Append player-visible match logs without modifying the shared game snapshot.
