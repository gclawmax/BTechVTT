...
const CUSTOM_ELECTRONICS = Object.freeze({
  case:{name:'CASE',tech:'inner_sphere',weight:.5,slots:1,label:'CASE'},
...
function renderMechDesigner() {
  ...
  <section class="designer-card designer-wide"><div class="designer-section-title"><h3>Weapons</h3><button onclick="addCustomWeapon()">Add weapon</button></div>${weaponRows || '<div class="roster-empty">No weapons fitted.</div>'}<div class="designer-section-title"><h3>Ammunition</h3><button onclick="addCustomAmmo()">Add ammunition</button></div>${ammoRows || '<div class="roster-empty">No ammunition bins fitted.</div>'}<div class="designer-section-title"><h3>Electronics</h3><button onclick="addCustomElectronic()">Add electronics</button></div>${electronicRows || '<div class="roster-empty">No electronic systems fitted.</div>'}<div class="designer-note">${clan ? 'Clan CASE protection is integral in this VTT: ammunition explosions are contained without adding equipment.' : 'Add CASE in the same location as ammunition to vent an Inner Sphere ammunition explosion. CASE weighs 0.5 t and uses one critical slot.'} Targeting Computers are sized automatically from eligible direct-fire weapon tonnage. C3 networks are assigned in the match lobby.</div></section>
