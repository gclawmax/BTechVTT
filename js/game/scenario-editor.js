...
function generateScenarioEditorMap() {
  if (!scenarioEditorState) return;
  const seed = String(document.getElementById('scenario-seed')?.value || `${Date.now()}`); const density = Number(document.getElementById('scenario-density')?.value || 25) / 100;
  ...
  for (let row = 0; row < rows; row++) for (let col = 0; col < cols; col++) {
    ...
    if (roll < density * .36) terrain[code] = 'light_woods'; else if (roll < density * .52) terrain[code] = 'rough'; else if (roll < density * .61) terrain[code] = 'heavy_woods'; else if (roll < density * .68) terrain[code] = 'shallow_water'; else if (roll < density * .73) terrain[code] = 'rubble';
