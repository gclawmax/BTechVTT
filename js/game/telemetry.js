...
function reportBattleFacts(report = {}, stats = {}) {
  const completed = report.completed_at ? new Date(report.completed_at) : null;
  const completedText = completed && !Number.isNaN(completed.getTime()) ? completed.toLocaleString() : 'Not recorded';
  const eventCount = reportNum(report.event_count || stats.event_count);
  const catalogue = report.catalogue_version || 'Not recorded';
  return `<div class="victory-fact-grid"><article><span>Recorded events</span><b>${eventCount}</b><small>Sealed authoritative timeline</small></article><article><span>Completed</span><b>${escapeHtml(completedText)}</b><small>Local time</small></article><article><span>Catalogue</span><b>${escapeHtml(catalogue)}</b><small>Pinned for this match</small></article></div>`;
}
