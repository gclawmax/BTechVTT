...
async function handleCreateConfiguredVsAI() {
  const mapId = document.getElementById('vs-ai-map-select')?.value;
  const dropshipTonnage = Number(document.getElementById('vs-ai-tonnage-select')?.value);
  ...
  await createVsAIGame({ mapId, dropshipTonnage, victoryMode, ruleset, difficulty:aiDifficulty, personality:aiPersonality });
}

async function createVsAIGame({ mapId, dropshipTonnage, victoryMode = 'annihilation', ruleset = 'advanced_3060', difficulty = aiDifficulty, personality = aiPersonality, customScenario = null }) {
  if (!currentUser || (!BT_MAPS[mapId] && !BT_CUSTOM_MAPS[mapId]) || !VS_AI_FORCE_LIMITS.includes(Number(dropshipTonnage))) return;
  showLoading(true);
  try {
    ...
    const humanRoster = buildVsAiSuggestedForce(entries, Number(dropshipTonnage), `${aiSeed}:human`);
    const aiRoster = buildVsAiSuggestedForce(entries, Number(dropshipTonnage), `${aiSeed}:ai`, new Set(humanRoster));
    const setupState = { map_id:mapId, map_dimensions:mapDimensions(mapId), dropship_tonnage:Number(dropshipTonnage), ruleset:validRuleset, victory_mode:validMode, objective_hexes:validMode === 'control' ? objectiveHexesForMap(mapId) : [], objective_scores:{ '1':0, '2':0 }, vs_ai_mode:true, ai_difficulty:AI_DIFFICULTY_KEYS.includes(difficulty) ? difficulty : 'beginner', ai_personality:AI_PERSONALITY_KEYS.includes(personality) ? personality : 'balanced', ai_seed:aiSeed, ai_engine_version:BT_AI_ENGINE_VERSION, ai_decisions:[], catalogue_version:catalogueVersion, special_ammo_setup_v1:true, hidden_units_v1:true, minefield_allowance:{ '1':2, '2':2 }, minefields:[], rosters:{ '1':humanRoster, '2':aiRoster }, deployment_positions:{}, units:[], turn:0, phase:'setup' };
    ...
    setupState.ai_setup = { version:'gm2', generated_force:[...aiRoster], generated_deployment:[...setupState.deployment_positions['2']], minefields:'deferred-to-gm4-private-setup' };
